...
### Content Review Findings (December 2024)

The following issues were identified during content review and should be addressed in future implementations:

#### High Priority Issues

1. **Dynamic Email Template Content**

   - **Issue**: `estimate-email-content.njk` uses hardcoded placeholder text instead of dynamic session data
   - **Fix**: Replace hardcoded text with dynamic variables:
     - EDP area: `{{ data.redlineBoundaryPolygon.intersectingCatchment or 'Thames Valley EDP' }}`
     - Building counts: `{{ data.residentialBuildingCount }}`, `{{ data.roomCounts.hotelCount }}`, etc.
   - **Location**: `src/server/forms/nrf-estimate-1/views/estimate-email-content.njk`

2. **Missing Planning Reference Field**

   - **Issue**: Users selecting "I have a planning reference and I'm ready to pay" cannot provide their planning reference
   - **Fix**: Add planning reference input field for payment journey users
   - **Location**: New field needed in journey flow

3. **Summary Page Label Inconsistency**
   - **Issue**: Uses "Number of HMO rooms" instead of specification's "Number of multiple occupation rooms"
   - **Fix**: Update label to match specification exactly
   - **Location**: `src/server/forms/nrf-estimate-1/views/summary.njk`

#### Medium Priority Issues

4. **EDP Boundary Checking**

   - **Issue**: Controller uses hardcoded `isWithinEDP = true` instead of real boundary validation
   - **Fix**: Implement actual EDP boundary checking logic
   - **Location**: `src/server/forms/nrf-estimate-1/controller.js`

5. **Placeholder Contact Information**

   - **Issue**: All contact details use placeholders (XXXX, xxxxx@defra.gov.uk, 00000000000)
   - **Fix**: Replace with real contact information for production
   - **Location**: Multiple view files

6. **Admin Charge Amount**
   - **Issue**: Email template shows "£00" for admin charge
   - **Fix**: Update to actual admin charge amount
   - **Location**: `src/server/forms/nrf-estimate-1/views/estimate-email-content.njk`
