...
parameters:
  - name: journey
    ...
    type: string
  - name: spec-file
    description: Path to the specification markdown file (e.g., prompts/prototype-1/01-quote-journey-v4.md)
    required: true
    type: string
  - name: show-matches
...
This slash command validates that an implemented journey matches its specification by performing comprehensive comparison of page content, data fields, error messages, conditional logic, and user journey flow.

The command validates the journey structure at `src/server/forms/{journey}/`.
...
```bash
/content-review journey:nrf-estimate-1 spec-file:prompts/prototype-1/01-quote-journey-v4.md show-matches:true
```
...
- ✅ Session data handling (`request.yar.get()` / `request.yar.set()`)
- ✅ Back link functionality

**Note**: Controllers are validated from `src/server/forms/{journey}/controller.js`.
...
❌ GET /nrf-estimate-1/email - Missing handler
❌ POST /nrf-estimate-1/email - Missing handler
⚠️ POST /nrf-estimate-1/summary - No validation logic found
```

**Controller Coverage** (`controller.js`):

```
✅ GET/POST handlers implemented for all pages
❌ Missing handlers for 2 unimplemented pages
⚠️ POST summaryController needs validation logic
```

**Route Registration** (`index.js`):

```
✅ All controllers imported, GET routes registered
...
```
✅ request.yar.set()/get() used correctly
✅ Email format validation, flash messages used
❌ request.yar.reset() not called on completion
⚠️ Some fields not stored in session, validation incomplete
```
...
```
✅ Templates extend layouts/page.njk, GOV.UK macros used
✅ Error summaries, back links use govukBackLink
...
1. **Read each .njk file** in the views directory
2. **Extract page title**: Look for `{{ pageTitle }}` or `{{ heading }}` variable
3. **Parse form fields**: Find all `govukInput`, `govukTextarea`, `govukSelect` macros
4. **Check field names**: Extract `name` attributes from macro parameters
5. **Validate components**: Verify GOV.UK macro usage
6. **Find error messages**: Look for `errorMessage` in macros or `{{ error }}` variables
7. **Check conditional content**: Look for `{% if %}` blocks

### How to Validate Controllers

1. **Check for controller file**: Look for `src/server/forms/{journey}/controller.js`
2. **If found**:
   - Parse all GET and POST handler exports
   - Match handlers to pages in specification
   - Check validation logic in POST handlers
   - Verify `request.yar.get()` / `request.yar.set()` usage
   - Analyze conditional routing and redirect logic using `h.redirect(ROUTES.X)`
3. **Check route registration**: Verify `index.js` registers all routes
4. **If not found**: Report missing controller file
...
- ✅ **Routes complete**: All GET/POST routes implemented and registered in index.js
- ✅ **Validation present**: Form validation in POST controllers
- ✅ **Conditional logic**: All conditional flows implemented
- ✅ **GOV.UK compliant**: Proper component usage, templates extend layouts/page.njk
